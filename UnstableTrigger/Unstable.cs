...
namespace UnstableTrigger
{
    public class Unstable
    {
        ...
        {
            if (state)
                SendMessage(HWND, WM_LBUTTONDOWN, 1, 0); //mouse_event(MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0);
            else
                SendMessage(HWND, WM_LBUTTONUP, 0, 0);//mouse_event(MOUSEEVENTF_LEFTUP, 0, 0, 0, 0);
        }
        public void Jump(bool state)
        {
            if (state)
            {
                SendMessage(HWND, 256, 32, 3735553);
                
            }
            else
                SendMessage(HWND, 257, 32, 3735553);
        }
        ...
        public int LocalPlayerInstance;
